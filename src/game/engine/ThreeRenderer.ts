...
export class ThreeRenderer {
  ...
  private updateBuildings(state: GameState) {
    ...
    for (const b of state.buildings) {
      ...
      if (!group || group.name !== stateKey || (group.children.length === 0 && ModelRegistry.getInstance().isReady)) {
        if (group) {
  ...
  private create3DBuilding(b: Building): THREE.Group {
    ...
    // The Royal Palace uses its bespoke multi-tiered architectural castle fortress
    if (b.type === 'palace') {
      return this.createPalaceFortress(b, w, h);
    }

    // Guilds and town structures load high-quality GLTF models from ModelRegistry
    {
      ...
      if (gltfBuilding) {
        ...
        // Researched upgrades dressing (banners, ballistae, crystals...)
        const topY = -box.min.y * scale + 0.1 + size.y * scale;
        this.addUpgradeDressing(group, b, w, h, topY);
        return group;
      }

      // While models are still streaming behind the loading veil, return empty placeholder
      if (!ModelRegistry.getInstance().isReady) {
        return group;
      }

      // Robust fallback so no building is ever invisible if a model download fails
      return this.createBuildingFallback(b, w, h);
    }
  }

  /** Gable roof with triangular pediments and pitched slopes */
  private createGableRoof(width: number, depth: number, height: number, mat: THREE.Material, ridgeAlongX = false): THREE.Group {
    ...
    if (ridgeAlongX) {
      const RX1 = [-w2, h, 0], RX2 = [w2, h, 0];
      pushTri(A, RX1, RX2, [0, 0], [0, 1], [1, 1]);
      pushTri(A, RX2, B, [0, 0], [1, 1], [1, 0]);
      pushTri(C, RX2, RX1, [1, 0], [1, 1], [0, 1]);
      pushTri(C, RX1, D, [1, 0], [0, 1], [0, 0]);
      pushTri(A, D, RX1, [0, 0], [1, 0], [0.5, 1]);
      pushTri(B, RX2, C, [0, 0], [0.5, 1], [1, 0]);
    } else {
  ...
  /**
   * Sovereign Castle Fortress (Authentic Granite Ashlar Bricks, Crenellations,
   * Royal Pennants, Arched Gatehouse, Portcullis & Central Throne Keep)
   */
  private createPalaceFortress(b: Building, w: number, h: number): THREE.Group {
    const group = new THREE.Group();
    ...
    if (level >= 2) {
      ...
      beacon.userData.baseY = keepH + keepRoofH + 11;
      group.add(beacon);
    }
  ...
  /** Clean fallback building model so no structure is ever an empty void */
  private createBuildingFallback(b: Building, w: number, h: number): THREE.Group {
    const group = new THREE.Group();
    const stoneMat = new THREE.MeshStandardMaterial({ color: 0x64748b, map: this.royalCastleWallTexture, roughness: 0.85 });
    const woodMat = new THREE.MeshStandardMaterial({ color: 0x78350f, map: this.timberLogTexture, roughness: 0.85 });
    const roofMat = new THREE.MeshStandardMaterial({ color: 0x1e3a8a, roughness: 0.6 });
    ...
    const roof = this.createGableRoof(baseW * 1.1, baseD * 1.1, 10, roofMat);
    roof.position.y = baseH;
    group.add(roof);

    const door = new THREE.Mesh(new THREE.BoxGeometry(4, 7, 0.4), woodMat);
    door.position.set(0, 3.5, baseD / 2 + 0.1);
    group.add(door);

    this.addBannerPair(group, w, h, 0x1e3a8a, 0xfbbf24, 0);
    return group;
  }
  ...
  private updateLairs(state: GameState) {
    ...
    for (const lair of state.lairs) {
      ...
      if (!group || (group.children.length === 0 && ModelRegistry.getInstance().isReady)) {
        if (group) {
          this.scene.remove(group);
        }
  ...
  private create3DLair(lair: MonsterLair): THREE.Group {
    ...
    if (lair.type === 'graveyard') {
      ...
      } else if (ModelRegistry.getInstance().isReady) {
        const darkStoneMat = new THREE.MeshStandardMaterial({ color: 0x1e1b4b, map: this.royalCastleWallTexture, roughness: 0.9 });
        const keepGeo = new THREE.BoxGeometry(w * 0.7, 26, h * 0.7);
        const keep = new THREE.Mesh(keepGeo, darkStoneMat);
        keep.position.y = 13;
        keep.castShadow = true;
        group.add(keep);
        const roofGeo = new THREE.ConeGeometry(w * 0.45, 14, 8);
        const roof = new THREE.Mesh(roofGeo, new THREE.MeshStandardMaterial({ color: 0x450a0a }));
        roof.position.y = 33;
